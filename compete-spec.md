...
>
> **Update (2026-07-23) — YouTube phase un-deferred, two-tier spec written: see `sc7-yt-spec.md` (`SC-7-YT`).** The "paid provider" half of the blocker proved false for YouTube. The design splits the cheap *check* from the heavy *dive*: **Phase 1 — Presence Check** runs inside serp-compete via the **official YouTube Data API** (a sanctioned, quota-bounded HTTPS call like the existing DataForSEO/Moz clients + an env-only key — no yt-dlp, no 429 cooldowns, ToS-clean), auto-discovering which competitors have channels; **Phase 2 — Attention Dive** does the transcript + brand-mention-SoV work in the `ptd` app (free `yt-dlp` binary; verified no `googleapis`/`youtube/v3` in it) and serp-compete **consumes its export** like the C1 AV export, never scraping. Phase 1 is self-contained and shippable alone; Phase 2 is gated on Phase 1's confirmations. This **retires the old "do competitors have channels?" decision** — the tool measures it. **All other platforms (TikTok, Instagram, Reddit, podcasts) remain DEFERRED** — their providers really are paid. Awaiting build approval; the only open sub-decision is the Phase-2 mention corpus profile (Phase 1 unblocked).
...
**Freshness and run scope.** Tool 1 caches per domain for up to 30 days, so a handoff's anchors can be older than the run reading them and can outlive a competitor's place in the SERP. Three things follow. The anchor path reuses the handoff **already validated** in `get_latest_market_data` rather than re-reading the file unchecked — only the first read was schema-checked, so the second let the `moz` block bypass validation. `moz.generated_at` travels into every signal's evidence as `collected_at`, so a stale signal cannot read as a fresh observation. And `handoff_moz.restrict_to_run` intersects the block with this run's competitor set plus the client, so a domain that has since dropped out collects no finding.

**Coverage persistence.** `anchor_coverage` (run-keyed table, `INSERT OR REPLACE`, written even when empty) records how the fetch went. `reporting.py` reads it from the DB like every other section reads its own data, rather than being handed it by the caller — which is what lets an **old** run's report still say what could not be read. An empty result means the run predates the table, **not** that the run was clean, and the reader must not render a clean claim from it.

**Coverage reporting.** `handoff_moz.anchor_coverage` counts `with_anchors / read_no_anchors / no_record / errored / skipped / unknown` per domain. `read_no_anchors` — read successfully, genuinely no anchor text — is **not** unreadable; bucketing it `unknown` put a caveat warning of untrustworthy data on a clean run while naming no cause. `risk_radar.anchor_data_unreadable` is the single definition of "could not be read", used by both the section gate and the caveat so a new bucket cannot land in one and not the other. A total failure of the anchor path returns a `fetch_status: "unavailable"` sentinel, distinguishable from a schema-1.0 handoff where nothing was attempted and silence is correct, and the report prints a caveat whenever any domain was unreadable. A domain Tool 1 capped or dropped for quota carries no `anchor_texts` key at all, so its status is read from the domain block — counting it as `no_record` would turn "we ran out of row budget" into "we looked and found nothing".
...
**Own-site coverage.** The client is excluded from `moz.domains` by design, so Tool 1 sends its anchor text in the `moz.client.anchor_texts` entry instead (`moz.competitor.client_anchor_texts`, on). `anchor_texts_by_domain` folds that entry into the map it returns, so an own-site hit is tagged `is_own_site` and separated from competitor intel in the report. This is the case that reveals a **negative-SEO campaign aimed at the client** — someone else pointing spam links at you — which is why it is worth the extra anchor page. A client entry carrying only Brand Authority does not inflate the anchor coverage total.
