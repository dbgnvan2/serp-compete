...
>
> **Update (2026-07-23) — YouTube phase un-deferred, spec written: see `sc7-yt-spec.md` (`SC-7-YT`).** The "paid provider" half of the blocker proved false for YouTube: the `ptd` app already does YouTube discovery + transcript retrieval with the free `yt-dlp` binary — no API key, no quota (verified: no `googleapis`/`youtube/v3` references in it). serp-compete **consumes a `ptd` export** and never scrapes, so it gains no new runtime dependency and 429 cooldowns can't stall `run_audit()`. **All other platforms (TikTok, Instagram, Reddit, podcasts) remain DEFERRED under the original reasoning above** — their providers really are paid. `SC-7-YT` is awaiting owner approval and three decisions (ToS posture, whether competitors actually run channels, and the mention-SoV corpus profile).
